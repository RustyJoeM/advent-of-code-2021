...
fn solve(steps: usize, (starting, transitions): &(Vec<char>, Transitions)) -> Res {
    ...
    for _ in 0..steps {
        ...
        for (key, &cnt) in counts.iter() {
            ...
            if let Some(&ch) = transitions.get(key) {
                let entry = next_counts.entry((key.0, key.1)).or_insert(cnt);
                *entry -= cnt;
                // dbg!("removing", (key.0, key.1));
                let entry = next_counts.entry((key.0, ch)).or_insert(0);
                *entry += cnt;
                let entry = next_counts.entry((ch, key.1)).or_insert(0);
                *entry += cnt;
                // dbg!("adding", (key.0, ch), (ch, key.1));
            };
        }
    ...
    for (key, val) in counts.iter() {
        let entry = letter_counts.entry(key.0).or_insert(0);
        *entry += val;
    }
    let entry = letter_counts.entry(*last_letter).or_insert(0);
    *entry += 1;
